  ...
  return (
    <div className="w-full h-4/5 p-4 overflow-auto flex flex-col gap-8">
      {messages.map((message) => (
        <div
          key={message.id}
          className="w-full sm:w-3/4 flex flex-col gap-y-3 rounded-xl border-2 border-slate-200 dark:border-slate-700 p-4"
        >
          <div className="flex justify-between dark:text-white">
            <h3 className="truncate w-28">{message.username}</h3>
            <small className="">{convertDate(message.date)}</small>
          </div>
